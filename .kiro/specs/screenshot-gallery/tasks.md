...
- [ ] 1. Fix screenshot ordering in backend queries
  - [ ] 1.1 Fix tRPC `getByAppSlug` to preserve `screenshotIds` order
  ...
  - [ ] 1.2 Fix REST `GET /api/screenshots/app/:appSlug` to preserve order
...
- [ ] 2. Add thumbnail size optimization
  - [ ] 2.1 Add `size` query parameter to thumbnail image URLs in `ScreenshotPreview`
...
- [ ] 3. Fix Escape key layered dismissal
  - [ ] 3.1 Implement Escape key interception in fullscreen mode
  ...
  - [ ]\* 3.2 Write unit test for Escape key layering
...
- [ ] 4. Checkpoint - Verify all gaps resolved
