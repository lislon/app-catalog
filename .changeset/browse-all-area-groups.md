...
The full resource list is grouped into areas of squarish cards instead of one flat A-Z list
...
biggest section first, each with a large title, a tool count and a grid of
squarish cards. The area titles carry the section, so the old "Browse all"
header above them is gone.
...
A card shows icon, name and a three-line description. Hovering it expands the
card downward to show the rest of the description; the expanded panel overlays
the row below rather than reflowing the grid. Keyboard focus expands it the same
way, and `prefers-reduced-motion` drops the animation.
